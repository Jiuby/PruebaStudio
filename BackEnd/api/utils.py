...
def send_order_confirmation_email(order):
    ...
    for item in order.items.all():
        ...
        <tr>
            ...
            <td style="padding: 10px; border-bottom: 1px solid #333; color: #e5e5e5;
                text-align: right;">${item.price:,.0f}</td>
        </tr>
        """
    ...
    <head>
        <style>
            body {{ font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;
                   background-color: #000000; color: #ffffff; margin: 0; padding: 0; }}
            ...
            .logo {{ font-size: 24px; font-weight: 900; letter-spacing: -1px; color: #ffffff;
                    text-transform: uppercase; font-style: italic; text-decoration: none; }}
            .content {{ padding: 40px 20px; }}
            .order-info {{ margin-bottom: 30px; border: 1px solid #333; padding: 20px;
                          border-radius: 4px; background-color: #1a1a1a; }}
            .table {{ width: 100%; border-collapse: collapse; margin-bottom: 30px; }}
            .footer {{ background-color: #000000; padding: 20px; text-align: center;
                      font-size: 12px; color: #666; border-top: 1px solid #333; }}
            .button {{ display: inline-block; padding: 15px 30px; background-color: #ffffff;
        ...
        <div class="container">
            ...
            <div class="content">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            <th style="text-align: left; padding: 10px; border-bottom: 1px solid #555;
                                color: #888; font-size: 12px; text-transform: uppercase;">Producto</th>
                            <th style="text-align: center; padding: 10px; border-bottom: 1px solid #555;
                                color: #888; font-size: 12px; text-transform: uppercase;">Cant.</th>
                            <th style="text-align: right; padding: 10px; border-bottom: 1px solid #555;
                                color: #888; font-size: 12px; text-transform: uppercase;">Precio</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items_html}
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="2" style="padding: 15px 10px; text-align: right;
                                font-weight: bold; color: #ffffff;">Total</td>
                            <td style="padding: 15px 10px; text-align: right;
                                font-weight: bold; color: #ffffff;">${order.total:,.0f}</td>
                        </tr>
                    </tfoot>
                </table>
