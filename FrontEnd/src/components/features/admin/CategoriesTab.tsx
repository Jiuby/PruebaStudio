...
export const CategoriesTab: React.FC = () => {
  ...
  const handleDeleteCategory = (cat: string) => {
    if (cat === 'All') return;
    if (window.confirm(`¿Estás seguro de que deseas eliminar la categoría "${cat}"? Los productos en esta categoría pueden necesitar actualización.`)) {
      deleteCategory(cat);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <h2 className="text-3xl font-black uppercase italic text-white">Gestión de Categorías</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Add Category Form */}
        <div className="bg-brand-dark/20 border border-brand-dark p-6 h-fit">
          <h3 className="text-white font-bold uppercase tracking-widest text-sm mb-6">Agregar Nueva Categoría</h3>
        ...
        <div className="bg-brand-dark/10 border border-brand-dark p-6">
          <h3 className="text-white font-bold uppercase tracking-widest text-sm mb-6">Categorías Existentes</h3>
