        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-12 md:pt-24 bg-black/80 backdrop-blur-sm">
          ...
          >
            <div className="flex justify-between items-center mb-8 border-b border-brand-dark pb-4">
              <h2 className="text-2xl font-black uppercase italic text-white">{collectionToEdit ? 'Editar Colección' : 'Nueva Colección'}</h2>
            ...
            <form onSubmit={handleSaveCollection} className="space-y-6">
              <div>
                <label className="block text-xs uppercase font-bold text-neutral-500 mb-2">Título</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs uppercase font-bold text-neutral-500 mb-2">Subtítulo</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs uppercase font-bold text-neutral-500 mb-2">Tamaño de Visualización</label>
                <select
                  value={formData.size}
                  onChange={e => setFormData({ ...formData, size: e.target.value as any })}
                  className="w-full bg-brand-dark/30 border border-brand-dark p-3 text-white focus:outline-none focus:border-brand-bone"
                >
                  <option value="small">Pequeño (1/2 ancho)</option>
                  <option value="medium">Mediano (Cuadrado)</option>
              ...
              <div className="space-y-2">
                <label className="block text-xs uppercase font-bold text-neutral-500">Vista Previa</label>
              ...
              <button type="submit" className="w-full bg-brand-bone text-brand-black py-4 font-black uppercase tracking-widest hover:bg-white transition-colors">
                {collectionToEdit ? 'Actualizar Colección' : 'Crear Colección'}
              </button>
            </form>
          </motion.div>
        </div>
